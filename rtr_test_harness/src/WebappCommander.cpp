...
bool WebappCommander::AcceptEula() {
  ...
  goal.signature = kDefaultEulaSignature;
  if (!CallRosAction<rtr_msgs::SetEULAAcceptedAction>("/SetEULAAccepted", goal)) {
    return false;
  }
  return true;
}

bool WebappCommander::InstallProject(const std::string& zip_path) {
...
bool WebappCommander::GetInstalledProjects(std::vector<std::string>& projects) {
...
bool WebappCommander::AddAllProjectsToDeconGroup(const std::string& dc_group_name) {
...
bool WebappCommander::UnloadGroup(const std::string& dc_group_name) {
  rtr_msgs::DeconGroupGoal goal;
  goal.group_name = dc_group_name;
...
bool WebappCommander::GetLoadedDeconGroup(rtr_msgs::DeconGroupInfo& loaded_group) {
...
bool WebappCommander::ClearApplianceDatabase() {
