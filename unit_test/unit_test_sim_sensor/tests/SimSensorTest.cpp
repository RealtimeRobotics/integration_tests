...
cv::Mat TestRender(const SimSensor::Ptr sensor, const RobotObserver::Ptr robot,
  ...
  SensorFrame::ConstPtr rendered_frame =
      testutils::GetRenderedImageFromSimSensor(sensor, ft);
  ...
  if (rendered_frame) {
    ...
    SensorIntrinsics2D::ConstPtr default_intr =
        SensorIntrinsics2D::CastConstPtr(
            sensor->GetDefaultSimulatedIntrinsics(ft));
...
void TestDepthAccuracy(const std::string &uid, const std::string &model_name,
  ...
  SimSensor::Ptr sensor = testutils::CreateSimSensor(
      uid, "", Eigen::Quaterniond(0.0, 1.0, 0.0, 0.0),
  ...
  for (const auto &config : configs) {
    ...
    SensorFrame::ConstPtr frame = testutils::GetRenderedImageFromSimSensor(
        sensor, SensorFrameType::SENSOR_FRAME_IMAGE_DEPTH);
    SensorFrameDepthImage::ConstPtr depth_frame =
        SensorFrameDepthImage::CastConstPtr(frame);
    ...
    EXPECT_TRUE(testutils::TestDepthImageAccuracy(depth_frame, 10.f, meshes,
                                                  poses, intersect_tolerance));
  }
}

// Move robot to joint configuration and check for fiducial
bool DetectFiducial(const RobotObserver::Ptr observer,
  ...
  SensorFrame::ConstPtr rendered_frame =
      testutils::GetRenderedImageFromSimSensor(sensor, ft);
  SensorFrameImage::ConstPtr image_frame =
      SensorFrameImage::CastConstPtr(rendered_frame);
  cv::Mat image;
...
float ComputeReprojError(const RobotObserver::Ptr observer,
                         const SimSensor::Ptr sensor, const SensorFrameType &ft,
                         const CalibrationTarget &target,
                         const std::vector<cv::Point2f> &corners) {
  RobotObserver::LinkPoseMap pose_map = observer->GetLinkPoses();
  const std::map<SensorFrameType, SensorIntrinsics::ConstPtr> intr_map =
      sensor->getIntrinsics();
  const std::map<SensorFrameType, SensorExtrinsics::ConstPtr> extr_map =
      sensor->getExtrinsics();
  ...
  fiducial_pose = fiducial_pose * target.info.flange_to_target *
                  target.info.target_to_origin;
  if (!intr_map.count(ft) || !extr_map.count(ft)) {
    RTR_ERROR("Failed to find intrinsics or extrinsics of frame type {}", ft);
    return Inf;
  }
  SensorIntrinsics2D::ConstPtr intr =
      SensorIntrinsics2D::CastConstPtr(intr_map.at(ft));
  ...
  return Calibrator::ComputeReprojError(corners, target.points,
                                        points_tf.ToEigen(), K, dist);
}
...
TEST(SimSensor, SetCalibration) {
  ...
  std::set<SensorFrameType> types =
      sensor->GetPersistentCalibrationFrameTypes();
  ...
  for (const auto &ft : types) {
    ...
    if (intrin_out.count(ft)) {
      SensorIntrinsics2D::ConstPtr in =
          SensorIntrinsics2D::CastConstPtr(intrin_in[ft]);
      SensorIntrinsics2D::ConstPtr out =
          SensorIntrinsics2D::CastConstPtr(intrin_out[ft]);
      EXPECT_EQ(*in, *out);
    }
...
TEST(SimSensor, UR10DepthAccuracy) {
  ...
  TestDepthAccuracy(uid, model_name, testutils::UR5_JOINTS,
                    intersect_tolerance);
}
...
TEST(SimSensor, FanucDepthAccuracy) {
  ...
  const int intersect_tolerance = 10;
  TestDepthAccuracy(uid, model_name, testutils::FANUC_JOINTS,
                    intersect_tolerance);
}
...
TEST(SimSensor, MelcoDepthAccuracy) {
  ...
  const int intersect_tolerance = 10;
  TestDepthAccuracy(uid, model_name, testutils::MELCO_JOINTS,
                    intersect_tolerance);
}
...
TEST(SimSensor, SensorModelDepthDifference) {
  ...
  SimSensor::Ptr d435_sensor = testutils::CreateSimSensor(
      "INTEL_REALSENSE_D435_000000000000", "", q, xyz);
  SimSensor::Ptr d435e_sensor = testutils::CreateSimSensor(
      "FRAMOS_REALSENSE_D435e_000000000000", "", q, xyz);
  SimSensor::Ptr d455_sensor = testutils::CreateSimSensor(
      "INTEL_REALSENSE_D455_000000000000", "", q, xyz);
  SimSensor::Ptr l515_sensor = testutils::CreateSimSensor(
      "INTEL_REALSENSE_L515_0000000000000000", "", q, xyz);

  RobotObserver::Ptr observer =
      testutils::CreateRobotObserver(testutils::UR5_MODEL_NAME);
  RobotMaskGenerator::MeshMap mesh_map;
  ASSERT_TRUE(testutils::CreateMeshMap(observer, mesh_map));
...
TEST(SimSensor, FiducialDetection) {
  ...
  // add robot
  RobotObserver::Ptr observer =
      testutils::CreateRobotObserver(testutils::UR10_MODEL_NAME);
  ...
  ASSERT_TRUE(
      LoadMesh("data/fiducial_plate.dae", fiducial_meshes, fiducial_colors));
  ...
  EXPECT_LT(ComputeReprojError(observer, sensor, ft, target, corners),
            REPROJ_TOLERANCE);
