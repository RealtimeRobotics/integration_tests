...
RosController::Ptr CreateRosController(const std::string& robot_type, const std::string& prefix) {
  RosController::Config config;
  testutils::GetRobotPathInfo(robot_type, config.urdf_path, config.base_link,
                              config.end_effector_link);
  EXPECT_TRUE(GetDefaultParamJSONString(robot_type, config.robot_params));
  config.robot_type = robot_type;
  config.ros_package_path = utils::ROS_PACKAGE_PATH;
  config.has_gripper = false;
  config.connection_type = RobotManagerInterface::ConnectionType::kInternalSim;
  config.stopping_speed_factor = 1.f;
  config.velocity_factors = Vec(6, 1.f);
  config.acceleration_factors = Vec(6, 1.f);
  config.jerk_factors = Vec(6, 1.f);
  config.topic_prefix = prefix;
  config.do_setup_rtr = false;
  return RosController::Create(config);
}

sensor_msgs::JointState CreateJointStateMessage(const JointConfiguration& config) {
...
rtr_control_ros::RobotStatus CreateRobotStatusMessage(const RobotManagerInterface::State state,
...
TEST(RosRobotObserverTest, ROSTopics) {
  ...
  for (const auto& config : testutils::MELCO_JOINTS) {
    ...
    RTR_INFO("current_config {}", current_config);
    RTR_ERROR("config {}", config);
