...
TEST(RobotProxy, CalibrationIKCalls) {
  ...
  RobotProxy::Ptr proxy =
      testutils::CreateRobotProxy(testutils::UR3_MODEL_NAME);
  ASSERT_TRUE(proxy);
  ...
  EXPECT_TRUE(ik_manager->Solve(
      test_pose, JointConfiguration({0.f, 0.f, 0.f, 0.f, 0.f, 0.f}), config));
