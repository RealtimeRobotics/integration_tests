...
MeshImageGenerator::Ptr SetupMeshImageGenerator(const SensorIntrinsics2D::ConstPtr intr,
  ...
  EXPECT_TRUE(generator->Init(intr->width(), intr->height(), near_clip, far_clip, ft));
...
void TestRobotAccuracy(const SensorIntrinsics2D::ConstPtr intr,
  ...
  for (const auto& config : configs) {
    ...
    pose_map["cylinder"] = Pose(Mat3(0.0, 0.0, 0.8509035, 0.525322), Vec3(0.5, 0.5, 0.25));
    pose_map["torus"] = Pose(Mat3(0, -0.9589243, 0, 0.2836622), Vec3(-0.6, -0.6, 0.3));
